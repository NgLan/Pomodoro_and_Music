...
function FieldIcon({ type }: { type: "email" | "name" | "password" }) {
  const Icon =
    type === "email" ? Mail : type === "name" ? UserRound : LockKeyhole;
  return (
    <Icon
      aria-hidden="true"
...
function DisplayNameField({ form }: Pick<Props, "form">) {
  ...
  return (
    <FormField
      ...
      render={({ field }) => (
        <FormItem>
          <FormLabel>{translate("DISPLAY_NAME_LABEL")}</FormLabel>
          <FormControl>
            <div className="relative">
              <FieldIcon type="name" />
              <Input
                className="pl-10"
                placeholder={translate("DISPLAY_NAME_PLACEHOLDER")}
                {...field}
              />
            </div>
          </FormControl>
    ...
    <FormField
      ...
      render={({ field }) => (
        <FormItem>
          ...
          {mode === "register" && (
            <FormDescription>{translate("TXT_PASSWORD_RULE")}</FormDescription>
          )}
          <FormMessage />
        </FormItem>
      )}
...
export function AuthFields({ form, mode }: Props) {
  return (
    <>
      {mode === "register" && <DisplayNameField form={form} />}
      <EmailField form={form} />
      <PasswordField form={form} mode={mode} />
    </>
  );
}
