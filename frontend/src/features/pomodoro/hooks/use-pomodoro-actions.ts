...
export function useConfigurationActions(
  ...
  const saveMutation = useMutation({
    mutationFn: (draft: PomodoroConfigurationRequestDto) =>
      editing
        ? updatePomodoroConfiguration(token, editing.id, draft)
        : createPomodoroConfiguration(token, draft),
  });
  ...
  const remove = async (id: string) => {
    ...
    await Promise.all([
      client.invalidateQueries({ queryKey: ["pomodoro", "configurations"] }),
...
export function useHistoryRecorder(token: string) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () =>
      void client.invalidateQueries({ queryKey: ["pomodoro", "history"] }),
