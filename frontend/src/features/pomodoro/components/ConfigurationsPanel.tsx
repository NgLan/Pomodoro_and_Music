  ...
  return (
    <section className="space-y-7" aria-labelledby="configurations-title">
      <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
        <div className="max-w-2xl">
          <span className="text-accent-pink text-sm font-extrabold uppercase">
            {translate("TXT_CONFIGS_EYEBROW")}
          </span>
          <h2 id="configurations-title" className="mt-2">
            {translate("TXT_CONFIGS_TITLE")}
          </h2>
          <p className="text-muted-foreground mt-2">
            {translate("TXT_CONFIGS_DESCRIPTION")}
          </p>
        </div>
        {emptyAction}
      </div>
      {configurations.length === 0 ? (
        <EmptyState
          title={translate("TXT_EMPTY_CONFIG_TITLE")}
          description={translate("TXT_EMPTY_CONFIG_DESCRIPTION")}
          action={emptyAction}
        />
      ) : (
