...
import { POMODORO_HISTORY_REPOSITORY, type PomodoroHistoryRepositoryInterface } from '../interfaces/pomodoro-history.repository.interface.js';
...
export class PomodoroHistoryService implements PomodoroHistoryServiceInterface {
  ...
  async create(userId: string, input: CreatePomodoroHistoryInput): Promise<PomodoroHistoryRecordOutput> {
    const configurationName = await this.getConfigurationName(userId, input.pomodoroId);
  ...
  private async getConfigurationName(userId: string, id?: string | null): Promise<string | null> {
