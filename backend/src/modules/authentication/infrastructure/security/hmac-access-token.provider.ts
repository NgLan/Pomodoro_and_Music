...
interface TokenPayload { exp: number; iat: number; sub: string }
...
export class HmacAccessTokenProvider implements AccessTokenProviderInterface {
  ...
  create(userId: string, expiresInSeconds: number): string {
    ...
    const payload = this.encode({ sub: userId, iat: now, exp: now + expiresInSeconds });
  ...
  private verifyHeader(encoded: string): void {
    try {
      const header = JSON.parse(Buffer.from(encoded, 'base64url').toString('utf8')) as { alg?: string; typ?: string };
  ...
  private verifySignature(content: string, signature: string): void {
    ...
    if (actual.length !== expected.length || !timingSafeEqual(actual, expected)) this.unauthorized();
  }
  ...
  private parsePayload(encoded: string): TokenPayload {
    try {
      ...
      if (!value.sub || !Number.isInteger(value.exp) || value.exp <= Date.now() / 1000) this.unauthorized();
      return value;
    } catch {
      this.unauthorized();
    }
  ...
  private unauthorized(): never {
    throw new BusinessException({ code: ErrorCode.UNAUTHORIZED, message: 'Access token is invalid or expired' });
  }
}
