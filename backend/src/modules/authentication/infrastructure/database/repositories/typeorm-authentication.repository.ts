...
import { toRefreshTokenDomain, toRefreshTokenPersistence } from '../mappers/refresh-token.mapper.js';
...
export class TypeOrmAuthenticationRepository implements AuthenticationRepositoryInterface {
  ...
  async findActiveUserById(id: string): Promise<User | null> {
    const entity = await this.users.findOne({ where: { id, status: UserStatus.ACTIVE } });
    return entity ? toUserDomain(entity) : null;
  }
  ...
  async createUserWithRefreshToken(user: User, token: RefreshToken): Promise<void> {
    try {
  ...
  async replaceRefreshToken(currentId: string, token: RefreshToken, at: Date): Promise<void> {
    await this.dataSource.transaction(async (manager) => {
      const result = await manager.createQueryBuilder(RefreshTokenOrmEntity, 'token')
        .update().set({ revokedAt: at })
        .where('id = :currentId AND revoked_at IS NULL AND expires_at > :at', { currentId, at })
        .execute();
  ...
  async revokeRefreshToken(tokenHash: string, at: Date): Promise<void> {
    await this.tokens.createQueryBuilder().update().set({ revokedAt: at })
      .where('token_hash = :tokenHash AND revoked_at IS NULL', { tokenHash }).execute();
  }
