...
import { AUTHENTICATION_REPOSITORY, type AuthenticationRepositoryInterface } from '../interfaces/authentication-repository.interface.js';
import type { AuthenticationServiceInterface } from '../interfaces/authentication-service.interface.js';
import { PASSWORD_HASHER, type PasswordHasherInterface } from '../interfaces/password-hasher.interface.js';
...
export class AuthenticationService implements AuthenticationServiceInterface {
  constructor(
    @Inject(AUTHENTICATION_REPOSITORY) private readonly repository: AuthenticationRepositoryInterface,
    @Inject(PASSWORD_HASHER) private readonly passwordHasher: PasswordHasherInterface,
  ...
  async register(input: RegisterInput): Promise<AuthenticationOutput> {
    ...
    const user = User.create({
      id: randomUUID(), email, displayName: input.displayName?.trim() || null,
      passwordHash: await this.passwordHasher.hash(input.password),
      authProvider: AuthProvider.LOCAL, providerSubject: null,
    ...
    return this.sessions.createOutput(user, refresh.rawToken, refresh.entity.expiresAt);
  }

  async login(input: LoginInput): Promise<AuthenticationOutput> {
    const email = input.email.trim().toLowerCase();
    const user = await this.repository.findUserByEmail(email);
    if (!(await this.hasValidCredentials(user, input.password))) this.invalidCredentials();
  ...
  async refresh(rawToken: string): Promise<AuthenticationOutput> {
    ...
    await this.repository.replaceRefreshToken(current!.id, replacement.entity, now);
    return this.sessions.createOutput(user!, replacement.rawToken, replacement.entity.expiresAt);
  }

  async logout(rawToken?: string): Promise<void> {
    if (rawToken) await this.repository.revokeRefreshToken(hashToken(rawToken), new Date());
  }
  ...
  private async hasValidCredentials(user: User | null, password: string): Promise<boolean> {
    if (!user || user.status !== UserStatus.ACTIVE || !user.passwordHash) return false;
    return this.passwordHasher.verify(password, user.passwordHash);
  }

  private emailExists(): never {
    throw new BusinessException({ code: ErrorCode.EMAIL_ALREADY_REGISTERED, message: 'An account with this email already exists' });
  }
  private invalidCredentials(): never {
    throw new BusinessException({ code: ErrorCode.INVALID_CREDENTIALS, message: 'Email or password is incorrect' });
  }
  private invalidRefreshToken(): never {
    throw new BusinessException({ code: ErrorCode.INVALID_REFRESH_TOKEN, message: 'Refresh token is invalid or expired' });
  }
  private unauthorized(): never {
    throw new BusinessException({ code: ErrorCode.UNAUTHORIZED, message: 'Authentication is required' });
  }
}
